...
const VehicleSpecs = () => {
  ...
  // 3. Create a handler for the booking button
  const handleRentClick = () => {
    ...
    if (!token) {
      ...
      // User is logged in (User, Owner, or Admin)
      setIsModalOpen(true);
    }
  };
  ...
  const specs = [
    { label: "Gear Box", value: car.description?.transmission || "N/A", icon: "⚙️" },
    { label: "Fuel", value: car.description?.fuel || "N/A", icon: "⛽" },
    { label: "Seats", value: car.description?.seats || "N/A", icon: "💺" },
    { label: "AC", value: car.description?.Air_conditioner ? "Yes" : "No", icon: "❄️" },
    { label: "Air Bags", value: car.description?.Air_Bags || "N/A", icon: "🛡️" },
  ...
  return (
    <div className="w-screen mx-auto p-13 bg-white min-h-auto ">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mt-23 max-w-7xl mx-auto">
        ...
        <div>
          <div className="mb-6">
            <h1 className="text-4xl font-black text-gray-900 uppercase">{car.carName}</h1>
            ...
            {car.images?.map((img, i) => (
              <div
                ...
                className={`w-24 h-20 bg-gray-50 rounded-xl overflow-hidden cursor-pointer border-2 transition-all shrink-0 ${
                  mainImage === img ? "border-green-500 scale-95" : "border-transparent"
                }`}
              >
        ...
        <div className="flex flex-col gap-8">
          <section>
            <h3 className="text-lg font-bold text-gray-800 mb-4">Technical Specification</h3>
          ...
          {/* 4. Updated Button to use handleRentClick */}
          <button
          ...
          <section>
            <h3 className="text-lg font-bold text-gray-800 mb-4">Car Equipment</h3>
